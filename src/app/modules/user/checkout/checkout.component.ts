...
export class CheckoutComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // CHECK IF DISHES OR WEEKELYdEAL

    this.cartService.MyCart(this.userService.currentUser.id).subscribe(res => {
      if (res.Data.dishes != null) {
        this.isWeekelyDeal = false;
      } else {
        this.ShowDays = res.Data.weeklydeals[0].days;
        // push
        res.Data.weeklydeals[0].days.forEach(element => {
          this.Day.push(element.id);
        });
        this.isWeekelyDeal = true;
      }
    });
  }
  ...
  close() {}
  save() {
    this.form.get('days').setValue(this.Day);
    if (
      moment(this.form.value.date).format('YYYY-MM-DD') <
      moment(new Date()).format('YYYY-MM-DD')
    ) {
      this.toastr.error('you cannot select Past Date');
    } else if (this.Day.length <= 0 && this.isWeekelyDeal == true) {
      this.toastr.error('you must select one Day at least');
